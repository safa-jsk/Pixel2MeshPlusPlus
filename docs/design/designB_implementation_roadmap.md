...
All necessary infrastructure files have been created in `env/gpu/`:

```
env/gpu/
├── QUICKSTART.md                  ← Start here for quick setup
├── Dockerfile                     ← Option 1: Docker container
├── requirements_gpu.txt           ← Python packages
├── build_ops.sh                   ← Build CUDA ops (main script)
├── setup_and_verify.sh            ← One-command full setup
└── benchmark.sh                   ← A vs B benchmarking
```
...
**If build fails:** See troubleshooting section in [env/gpu/QUICKSTART.md](env/gpu/QUICKSTART.md)
...
docs/
├── designB_gpu_strategy.md          ← Strategy & approach
├── designB_commit.txt               ← Baseline commit hash (will be created)
└── designB_changes.md               ← Change log (will be created)

outputs/designB/
├── benchmark/
│   ├── design_a_times.txt          ← CPU baseline times
│   ├── design_b_times.txt          ← GPU times
│   ├── system_info.txt             ← GPU info
│   └── logs/                       ← Detailed logs
└── poster_figs/                     ← For thesis chapter
```
...
**Next Action:** Pick your environment option (Docker or Native) and run:

- Docker: `docker build -f env/gpu/Dockerfile -t p2mpp-gpu:latest .`
- Native: `pip install -r env/gpu/requirements_gpu.txt`
