...
_TF_SRC = _os.path.abspath(_os.path.join(_SCRIPT_DIR, '..'))
_PROJECT_ROOT = _os.path.abspath(_os.path.join(_TF_SRC, '..', '..', '..'))
if _TF_SRC not in _sys.path:
    _sys.path.insert(0, _TF_SRC)
...
def main(cfg):
    ...
    # ---------------------------------------------------------------
    print('=> load data')
    _assets = _os.path.join(_PROJECT_ROOT, 'assets')
    demo_img_list = [_os.path.join(_assets, 'demo_inputs', 'plane1.png'),
                     _os.path.join(_assets, 'demo_inputs', 'plane2.png'),
                     _os.path.join(_assets, 'demo_inputs', 'plane3.png')]
    img_all_view = load_demo_image(demo_img_list)
    cameras = np.loadtxt(_os.path.join(_assets, 'demo_inputs', 'cameras.txt'))
    ...
    pkl = pickle.load(open(_os.path.join(_assets, 'data_templates', 'iccv_p2mpp.dat'), 'rb'))
    # Construct Feed dict
    ...
    face = np.loadtxt(_os.path.join(_assets, 'data_templates', 'face3.obj'), dtype='|S32')
    mesh = np.vstack((vert, face))

    pred_path = _os.path.join(_PROJECT_ROOT, 'artifacts', 'outputs', 'temp', 'predict.obj')
    _os.makedirs(_os.path.dirname(pred_path), exist_ok=True)
