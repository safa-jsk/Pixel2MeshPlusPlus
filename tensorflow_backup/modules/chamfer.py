...
try:
    _nn_mod = tf.load_op_library('./external/tf_nndistance_so.so')
    _use_custom_op = True
    print("[INFO] Loaded custom NNDistance op: external/tf_nndistance_so.so")
except Exception as e:
    print("[WARN] Failed to load custom NNDistance op. Falling back to CPU implementation.")
    print("[WARN]", e)
    _use_custom_op = False

if not _use_custom_op:
    # CPU fallback (pure TF/Python). This exists in your repo.
    from external.tf_nndistance_cpu import nn_distance_cpu as _cpu_nn_distance
