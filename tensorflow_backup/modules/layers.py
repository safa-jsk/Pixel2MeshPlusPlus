...
class LocalGraphProjection(Layer):
    ...
    def _call(self, inputs):
        ...
        for i in range(self.view_number):
            ...
            x = h / (224.0 / 224)
            y = w / (224.0 / 224)
            x = tf.minimum(tf.maximum(x, 0), 223)
            y = tf.minimum(tf.maximum(y, 0), 223)
            out1 = self.bi_linear_sample(self.img_feat[0], n, x, y)
            out1_list.append(out1)
            x = h / (224.0 / 112)
            y = w / (224.0 / 112)
            x = tf.minimum(tf.maximum(x, 0), 111)
            y = tf.minimum(tf.maximum(y, 0), 111)
            out2 = self.bi_linear_sample(self.img_feat[1], n, x, y)
            out2_list.append(out2)
            x = h / (224.0 / 56)
            y = w / (224.0 / 56)
            x = tf.minimum(tf.maximum(x, 0), 55)
            y = tf.minimum(tf.maximum(y, 0), 55)
